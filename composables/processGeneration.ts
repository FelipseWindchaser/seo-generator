...
export class SEOGenerator {
  ...
  public async generateWithValidation(
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      console.log(
        `[SEOGenerator] Generation attempt ${attempt + 1}/${maxAttempts}`
      );
      ...
      try {
        if (attempt === 0) {
          ...
          const fixed = await this.fixContent(
            currentTitle,
            currentContent,
            validationResult,
            data
        ...
        additionalChecks = this.checkAdditionalRequirements(
          currentContent,
          data
        );
        ...
        console.error(
          `[SEOGenerator] Error in generation attempt ${attempt + 1}:`,
          error
        );
        throw error;
        // if (attempt === 0) {
        //     console.error("[SEOGenerator] Critical failure on first generation attempt. Aborting. error: " + (error as Error).message);
        //     throw error; 
        //   }
    ...
    throw new Error(
      "Generation failed after all attempts and result could not be prepared."
    );
  }
  ...
  - Ключей использовано: ${validation.metrics.keywordsUsed}/${
  ...
  private parseLLMResponse(responseText: string): {
    ...
    for (let i = 0; i < contentParts.length; i++) {
      if (
        contentParts[i].includes("ЗАГОЛОВОК") &&
...
export const processTask = async () => {
  ...
  for await (const task of tasks) {
    ...
    const generator = new SEOGenerator(apiKey);
    if (!task.request) {
      console.error("Task request is undefined for task", task.id);
      continue;
    }
    const result = await generator.generateWithValidation(task.request);
    console.log("result", result);
    // await generateTextFromTask(task);
    ...
    contents: [
      {
        parts: [
          {
            text: `Сгенерируй текст для товара ${
              task.request?.productUrl
            } с ключевыми словами ${task.request?.keywords.join(
              ", "
            )} с отзывами ${
              task.request?.reviews
            } и уникальными торговыми предложениями ${
              task.request?.usp
            }. Выведи в ответе ссылку на товар и опиши ее содержание.`,
          },
        ],
      },
    ],
  };
  console.log('keywords', task.request?.keywords);
